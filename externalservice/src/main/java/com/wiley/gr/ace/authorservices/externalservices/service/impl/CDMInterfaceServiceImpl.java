...
public class CDMInterfaceServiceImpl implements CDMInterfaceService {
	...
	public CDMAffiliation[] getAffiliationsForUser(String userId) {
		...
		try {
			...
			HttpEntity<CDMAffiliation[]> requestEntity = new HttpEntity<CDMAffiliation[]>(
					requestHeaders);

			ResponseEntity<CDMAffiliation[]> response = restTemplate.exchange(
					uri, HttpMethod.GET, requestEntity, CDMAffiliation[].class);
	...
	public CDMAffiliation editAffialiationsForUser(String userId,
		...
		return (CDMAffiliation) StubInvokerUtil.invokeStub(url, HttpMethod.GET,
				"CDMAffiliation");
	}
	...
	public Interests[] getInterests(String userId) {
		...
		try {
			...
			RestTemplate restTemplate = new RestTemplate();
			HttpHeaders requestHeaders = new HttpHeaders();
			HttpEntity<Interests[]> requestEntity = new HttpEntity<Interests[]>(
					requestHeaders);
	...
	public LookUpProfile lookUpProfile(String userId) {
		...
		try {
			...
			RestTemplate restTemplate = new RestTemplate();
			HttpHeaders requestHeaders = new HttpHeaders();
			HttpEntity<LookUpProfile> requestEntity = new HttpEntity<LookUpProfile>(
					requestHeaders);
			ResponseEntity<LookUpProfile> response = restTemplate.exchange(uri,
					HttpMethod.GET, requestEntity, LookUpProfile.class);
			return response.getBody();
