...
public class UserRolesDAOImpl implements UserRolesDAO {
	...
	/* (non-Javadoc)
	 * @see com.wiley.gr.ace.authorservices.persistence.services.UserRolesDAO#addOrUpdateUserRoles(com.wiley.gr.ace.authorservices.persistence.entity.UserRoles, java.util.List)
	 */
	@Override
	public void addOrUpdateUserRoles(Roles roles,
		...
		try {
			...
			for (RolePermissions rolePermissions : daoPermissionsList) {
				...
				daoPermissionsMap.put(rolePermissions.getId().getPermissionId(), rolePermissions);
				
			}
			
			for (Permissions permissions : permissionsList) {
				
				if(daoPermissionsMap.containsKey(permissions.getPermissionId())) {
			...
			for (Permissions permissions : addList) {
				...
				rolePermissions.setId(new RolePermissionsId(roles.getRoleId(), permissions.getPermissionId()));
				
				session.saveOrUpdate(rolePermissions);
				
			}
			
			for (Map.Entry<Integer, RolePermissions> entry : daoPermissionsMap.entrySet()) {
