...
//            .getLogger(SaveArticleDAOImpl.class);
     ...
     */
    /*
     * @Override public boolean saveArticleInvitation(InviteResetpwdLog invite)
     * throws Exception { Session session = null; try { session =
     * getSessionFactory().openSession(); session.getTransaction().begin();
     * session.save(invite); session.getTransaction().commit(); } catch
     * (Exception e) { if (session != null) {
     * session.getTransaction().rollback(); }
     * LOGGER.error("Exception while inserting record ..." + e.getMessage()); }
